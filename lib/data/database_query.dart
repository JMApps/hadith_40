...
class DatabaseQuery {
  ...
  Future<List<HadeethItem>> getAllHadeeths() async {
    ...
    List<HadeethItem>? allChapters =
        res.isNotEmpty ? res.map((c) => HadeethItem.fromMap(c)).toList() : null;
    return allChapters!;
  }
  ...
  Future<List<HadeethItem>> getSearchResult(String text) async {
    ...
    var res = await dbClient.rawQuery(
        "SELECT * FROM Table_of_chapters WHERE hadeeth_number LIKE '%$text%' OR hadeeth_title LIKE '%$text%'");
    List<HadeethItem>? searchResults =
        res.isNotEmpty ? res.map((c) => HadeethItem.fromMap(c)).toList() : null;
  ...
  Future<List<HadeethItem>> getFavoriteHadeeths() async {
    ...
    List<HadeethItem>? favoriteChapters =
        res.isNotEmpty ? res.map((c) => HadeethItem.fromMap(c)).toList() : null;
  ...
  Future<List<HadeethItem>> getHadeethContent(int id) async {
    var dbClient = await con.db;
    var res = await dbClient.query('Table_of_chapters', where: '_id == $id');
    List<HadeethItem>? chapterContent =
        res.isNotEmpty ? res.map((c) => HadeethItem.fromMap(c)).toList() : null;
  ...
  Future<List<HadeethApartItem>> getApartHadeeth(int hadeethId) async {
    var dbClient = await con.db;
    var res = await dbClient.query('Table_of_cut',
        where: 'item_position == $hadeethId');
    List<HadeethApartItem>? apartHadeeth = res.isNotEmpty
        ? res.map((c) => HadeethApartItem.fromMap(c)).toList()
        : null;
    return apartHadeeth!;
  }
}
